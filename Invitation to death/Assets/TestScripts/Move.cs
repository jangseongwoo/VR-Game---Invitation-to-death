...
public class Move : MonoBehaviour
{
	...
	void Update ()
	{
        ...
        if (Input.GetKey("z"))
            GetComponent<Rigidbody>().AddForce(new Vector3(0, 0, 1) * 10.0f);
        if (Input.GetKey("x"))
            GetComponent<Rigidbody>().AddForce(new Vector3(1, 0, 0) * 10.0f);
        if (Input.GetKey("y"))
            GetComponent<Rigidbody>().AddForce(new Vector3(0, 1, 0) * 10.0f);
