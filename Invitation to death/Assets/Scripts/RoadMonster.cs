...
public class RoadMonster : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider coll)
    {
        ...
        if (coll.GetComponent<Collider>().tag == "BULLET")
        {
            ...
            monsterHp -= coll.gameObject.GetComponent<BulletFire>().damage;
            if (monsterHp <= 0)
            {
                RoadMonsterDie();
            }
    ...
    void RoadMonsterDie()
    {
        ...
        //죽는 애니메이션 보이기도 전에 통과하길래 시간 지난 후 통과되도록 하려고 했는데 실패
        /*
        float limitTime = 0.0f;

        while (limitTime < 5.0f)
        {
